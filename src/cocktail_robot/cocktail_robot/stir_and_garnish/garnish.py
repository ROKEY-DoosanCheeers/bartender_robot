...
class GarnishAction(BaseAction):
    ...
    def execute(self):
        self.arm.movej(self.topping["pick"]) #### pos before grasp
        self.arm.movel(pos=[0,0,-40,0,0,0], mod=DR_MV_MOD_REL, ref=DR_BASE)
        self.arm.grasp()

        ##TODO consequence actions
        ...
        self.arm.movej(self.topping["stir"]) ##### pos before put in
        self.down_stir_up()
        self.arm.movej(self.topping["spoon_grasp_1"])
        self.arm.movej(self.topping["spoon_grasp_0"])
